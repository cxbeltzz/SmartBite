...
def initialize_recipe_database():
    ...
    if recipe_count > 0:
        print(f"Base de datos ya tiene {recipe_count} recetas. Saltando inicializacion...")
        return True
    
    print("Inicializando base de datos de recetas...")
    print("NOTA: Esto puede tardar 35-45 minutos debido añ tamaño del dataset de recetas")
...
def start_application():
    ...
    cmd = sys.argv[1:] if len(sys.argv) > 1 else ["gunicorn", "--bind", "0.0.0.0:5000", "app:app", "--workers", "2", "--threads", "4"]
    subprocess.run(cmd)
