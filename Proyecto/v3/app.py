...
def recommend_meals(
    calories_per_meal: float, meals_per_day: int
) -> Dict[str, List[dict]]:
    ...
    if request.method == "POST":
        try:
            user_values = {
                ...
                "activity_index": int(
                    request.form.get("activity_index", ACTIVITY_ORDER.index("moderate"))
                ),
                "plan": request.form.get("plan", "maintain"),
                "meals_per_day": int(request.form.get("meals_per_day", 3)),
            }
            activity_index = max(
                0, min(user_values["activity_index"], len(ACTIVITY_ORDER) - 1)
            )
    ...
    if request.method == "POST":
        try:
            for field in CUSTOM_NUTRIENT_FIELDS:
                ...
                defaults[field["name"]] = max(
                    field["min"], min(field["max"], float(raw_value))
                )

            recommendation_count = int(
                request.form.get("recommendation_count", recommendation_count)
            )
            recommendation_count = max(1, min(20, recommendation_count))

            ingredients_input = request.form.get("ingredients", "").strip()
            ingredient_terms = [
                token.strip() for token in ingredients_input.split(",") if token.strip()
            ]

            nutrient_vector = _build_custom_vector(defaults)
            dataframe = _get_dataframe()
            params = {
                "n_neighbors": max(recommendation_count, 1),
                "return_distance": False,
            }
            raw_results = model.recommend(
                dataframe, nutrient_vector, ingredients=ingredient_terms, params=params
            )
            recommendations = model.output_recommended_recipes(raw_results) or []
            if not recommendations:
                info_message = (
                    "No se encontraron recetas compatibles con esos valores. "
                    "Prueba ampliando los rangos o quitando filtros."
                )
        except Exception as exc:  # pylint: disable=broad-except
            error = str(exc)
    ...
    if request.method == "POST":
        ...
        if logged_user is not None:
            if logged_user.password:
                login_user(logged_user, remember=remember_me)
                return redirect("/")
            else:
