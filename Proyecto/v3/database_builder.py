...
def clear_existing_data(cursor) -> None:
    ...
    cursor.execute("TRUNCATE TABLE recipe_images RESTART IDENTITY CASCADE")
                ...
                for row in load_rows(csv_path):
                    ...
                    for position, ingredient in enumerate(ingredients):
                        ingredient_id = upsert_ingredient(
                            cursor, ingredient_cache, ingredient
                        )
                        cursor.execute(
                            """
                    ...
                    images = parse_quoted_list(row.get("Images"))
                    for position, image_url in enumerate(images):
                        cursor.execute(
                            """
                            INSERT INTO recipe_images(recipe_id, position, url)
                            VALUES (%s, %s, %s)
                            ON CONFLICT (recipe_id, position) DO UPDATE SET url = EXCLUDED.url
                            """,
                            (recipe_id, position, image_url),
                        )
    finally:
        connection.close()
